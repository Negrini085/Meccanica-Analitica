...
class DoppioPendolo : public FunzioneVettorialeBase{
	...
	DoppioPendolo() {m_m1=0; m_m2=0; m_l1=0; m_l2=0; }
	//Cotruttore che accetta in ingresso masse e lunghezze
	DoppioPendolo(double m1, double m2, double l1, double l2) {
		m_m1 = m1; m_m2 = m2; m_l1 = l1; m_l2 = l2;
	}
	...
	vector<double> Eval(double t, const vector<double>& stato) const override {
		...
		c = m_m2*m_l1*m_l2*stato[3]*sin((stato[0] - stato[1]))*(stato[2] - stato[3]) - m_m2*m_l1*m_l2*stato[2]*stato[3]*sin(stato[0] - stato[1]) - (m_m1 + m_m2)*acc_g*m_l1*sin(stato[0]);
		...
		/*double energia = 0.5*(m_m1 + m_m2)*pow(m_l1*stato[2], 2) + 0.5*m_m2*pow(m_l2*stato[3], 2) + m_m2*m_l1*m_l2*cos(stato[0] - stato[1])*stato[2]*stato[3] - (m_m1 + m_m2)*acc_g*m_l1*cos(stato[0]) - m_m2*acc_g*m_l2*cos(stato[1]);
